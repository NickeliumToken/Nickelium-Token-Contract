...
import "./ITether.sol";
import "./ProofOfReservesSummary.sol";

contract Mine4_NFT is ERC721Enumerable, ReentrancyGuard, Pausable {
    ...
    constructor() ERC721("Mine4_NFT", "NM4") {
	...
	ProofOfReservesSummary public proofOfReservesContract;
    ...
    function setContracts(                                                       
		...
		proofOfReservesContract = ProofOfReservesSummary(_proofofreserves);
    ...
    // Get all totals from Proof of Reserves (returns uint256, not enum)
    (uint256 landTotal, uint256 seaTotal, uint256 financeTotal) = proofOfReservesContract.getOriginTotals();
    
    if (nickelType == NickelType.Land) {  // 0
        uint256 maxAllowed = landTotal / 5;
        require(landStock + amount <= maxAllowed, "Exceeds 1/5 of Land reserves");
        landStock += amount;
        
    } else if (nickelType == NickelType.Sea) {  // 1
        uint256 maxAllowed = seaTotal / 5;
        require(seaStock + amount <= maxAllowed, "Exceeds 1/5 of Sea reserves");
        seaStock += amount;
        
    } else if (nickelType == NickelType.Financial) {  // 2 (matches Finance=2 in other contract)
        uint256 maxAllowed = financeTotal / 5;
        require(financialStock + amount <= maxAllowed, "Exceeds 1/5 of Finance reserves");
...
// Add getter functions for individual stock types
function getStockByType(NickelType nickelType) external view returns (uint256) {
    ...
    // Get total delivered Ni from Proof of Reserves (in tonnes) and convert to grams
    // 1 tonne = 1,000,000 grams
    uint256 maxLimitInGrams = (proofOfReservesContract.totalDeliveredNiT() * 1_000_000) / 5;
    
    require(newLimit <= maxLimitInGrams, "Your limit exceeds 1/5 of current delivered stock");
    stockLimit = newLimit;
