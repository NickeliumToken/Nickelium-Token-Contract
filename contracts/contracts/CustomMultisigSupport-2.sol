...
event SetStockLimitConfirmed(uint256 indexed actionId, address indexed owner);
    ...
    _removeSetStockLimitId(actionId);
    delete setStockLimitActions[actionId];

    emit SetStockLimitExecuted(actionId, action.newLimit);
}

function _removeSetStockLimitId(uint256 actionId) internal {
    for (uint256 i = 0; i < setStockLimitIndexes.length; i++) {
        if (setStockLimitIndexes[i] == actionId) {
            setStockLimitIndexes[i] = setStockLimitIndexes[setStockLimitIndexes.length - 1];
            setStockLimitIndexes.pop();
            break;
        }
    }
}

function deleteSetStockLimitAction(uint256 actionId) 
    public 
    onlyOwners 
    nonReentrant 
    whenNotPaused 
{
    SetStockLimitAction storage action = setStockLimitActions[actionId];
    require(action.newLimit > 0, "Action does not exist"); // Check if exists
    require(
        !action.isConfirmedByOwner1 || !action.isConfirmedByOwner2, 
        "Action already executed"
    );
