...
contract USDCorders is Pausable, ReentrancyGuard{
    ...
    // Buy orders separate queues for Ether and USDC
    SharedStructs.Order[] public buyOrdersUSDC;
    // Sell orders separate queues for Ether and USDC
    SharedStructs.Order[] public sellOrdersUSDC;
    
    // Reference to Nickelium and USDC contracts
  ...
  function getBuyOrdersUSDC() public view returns (SharedStructs.Order[] memory) {
...
function addBuyOrder(
	...
	require(_price >= centralContract.LMEprice(), "Order price must be at least the current LME price");
	 if (buyOrdersUSDC.length == 0) {
    // If the buyOrdersUSDC array is empty, execute matching logic with opposite orders (sellOrdersUSDC)
    matchNewOrderUSDC(newOrder, true, true); // true for buy order
} else {
    // If the buyOrdersUSDC array is not empty, check if the new order's price is higher than the first buy order's price
    if (newOrder.price > buyOrdersUSDC[0].price) {
        // Execute matching logic
        matchNewOrderUSDC(newOrder, true, true); // true for buy order
    } else {
        // Insert the new order into the correct position
        insertOrderInSortedPosition(buyOrdersUSDC, newOrder); // true for buy order
    }
}

}

    function matchNewOrderUSDC(SharedStructs.Order memory newOrder, bool isBuyOrder, bool isNewOrder) internal whenNotPaused {
    uint256 tradeAmount;
    uint256 remainingAmount = newOrder.amount;

    SharedStructs.Order[] storage oppositeOrders = isBuyOrder ? sellOrdersUSDC : buyOrdersUSDC;
    ...
    while (i < oppositeOrders.length) {
        ...
        executeTradeUSDC(newOrder, oppositeOrder, tradeAmount, isBuyOrder);
    ...
    SharedStructs.Order[] storage targetOrders = isBuyOrder ? buyOrdersUSDC : sellOrdersUSDC;
    ...
    if (remainingAmount > 0) {
        ...
        if (targetOrders.length >= 20) {
            if (isBuyOrder) {
                removeWorstPricedOrder(targetOrders, SharedStructs.PaymentMethod.USDC);
            } else {
   ...
   /*function matchNewOrderUSDC(SharedStructs.Order memory newOrder, bool isBuyOrder) internal whenNotPaused {
    ...
    if (isBuyOrder) {
        oppositeOrders = sellOrdersUSDC; // Match against sell orders
    } else {
        oppositeOrders = buyOrdersUSDC; // Match against buy orders
    }
    ...
    if (remainingAmount > 0) {
        ...
        if (isBuyOrder) {
            targetOrders = buyOrdersUSDC; // Insert into buy orders list
            ...
            targetOrders = sellOrdersUSDC; // Insert into sell orders list
...
function executeTradeUSDC(SharedStructs.Order memory newOrder, SharedStructs.Order storage oppositeOrder, uint256 tradeAmount, bool isBuyOrder) internal {
    ...
    // Transfer USDC payment to the seller
    escrowHandler.approveUSDC(paymentAmount);
    ...
    /*function insertOrderInSortedPosition(SharedStructs.Order[] storage orders, SharedStructs.Order memory newOrder) internal {
    uint256 n = orders.length;
    uint256 i = 0;
    ...
    // If there are already 20 orders, check if the last (worst-priced) order should be removed
    if (orders.length >= 20) {
        SharedStructs.Order memory lastOrder = orders[orders.length - 1];

        // If the new order is priced higher than the lowest-priced order, remove the last one
        if (newOrder.price > lastOrder.price) {
            removeWorstPricedOrder(orders, newOrder.priceCurrency);
        } else {
            // If the new order is not better than the worst, simply return without inserting it
            return;
        }
    }

    // Expand array by pushing a dummy value
    orders.push(orders[n - 1]); // Duplicate last element to create space

    // Choose the best shifting strategy
    if (i < n / 2) {
        // If inserting in the first half, shift elements forward (left to right)
        for (uint256 j = 0; j < i; j++) {
            orders[j] = orders[j + 1];
        }
    } else {
        // If inserting in the second half, shift elements backward (right to left)
    ...
    function insertSellOrderInSortedPosition(SharedStructs.Order[] storage orders, SharedStructs.Order memory newOrder) internal {
    uint256 n = orders.length;
    uint256 i = 0;

    // Find the correct insertion index (low prices first, equal prices below existing ones)
    while (i < n && orders[i].price <= newOrder.price) {
        i++;
    }
...
function removeWorstPricedOrder(SharedStructs.Order[] storage orders, SharedStructs.PaymentMethod paymentMethod) internal {
    ...
    // Refund the escrowed funds to the owner of the worst-priced order
        ...
        escrowHandler.approveUSDC(cost);
        USDCContract.transferFrom(address(escrowHandler), worstOrder.user, cost);
    ...
    require(_price >= centralContract.LMEprice(), "Order price must be at least the current LME price");
        if (sellOrdersUSDC.length == 0) {
    // If the sellOrdersUSDC array is empty, execute matching logic with opposite orders (buyOrdersUSDC)
    matchNewOrderUSDC(newOrder, false, true); // false for sell order
} else {
    // If the sellOrdersUSDC array is not empty, check if the new order's price is higher than the first buy order's price
    if (newOrder.price < sellOrdersUSDC[0].price) {
        // Execute matching logic
        matchNewOrderUSDC(newOrder, false, true); // false for sell order
    } else {
        // Insert the new order into the correct position
        insertSellOrderInSortedPosition(sellOrdersUSDC, newOrder); // true for buy order
    }
}
}
    ...
    for (uint256 i = 0; i < buyOrdersUSDC.length; i++) {
        if (buyOrdersUSDC[i].orderID == orderID) {
            ...
            if (newCost > oldCost) {
                ...
                escrowHandler.approveUSDC(refund);
                USDCContract.transferFrom(address(escrowHandler), order.user, refund);
            ...
            if (sellOrdersUSDC.length > 0 && newPrice >= sellOrdersUSDC[0].price) {
                matchNewOrderUSDC(updatedOrder, true, false); // Call match function
            }
    ...
    for (uint256 i = 0; i < sellOrdersUSDC.length; i++) {
        if (sellOrdersUSDC[i].orderID == orderID) {
           ...
           // Update the price of the order
            ...
            repositionOrder(sellOrdersUSDC, i, true);

            // Match orders only if the new price is <= best available buy order price
            if (buyOrdersUSDC.length > 0 && newPrice <= buyOrdersUSDC[0].price) {
                matchNewOrderUSDC(updatedOrder, false, false); // Call match function
            }
